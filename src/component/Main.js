...
class Main extends Component {
  ...
  getMovies = async () => {
    ...
    console.log(movies); //콘솔창에 데이터 출력 확인

    this.setState({movies, isLoading:false});
  
  }; 

  //생명주기함수로서 화면 render링이 끝나고 나서 실행되는 함수
  componentDidMount() {
    // setTimeout(() => {
  ...
  render() {
    ...
    return (
      <div>
        ...
        {/* 방법2. 이미지로 표현 */}
        {/* {isLoading ? <img src={`${process.env.PUBLIC_URL}/images/bx_loader.gif`} alt="로딩 중" /> : '데이터 로딩 완료. 준비끝~'} */}

        {isLoading ? 
         <section className="loading-container2">
          <img
            src={`${process.env.PUBLIC_URL}/images/bx_loader.gif`}
            alt="로딩 중"
            className="loading_img2"
          />
          </section>
        : (
